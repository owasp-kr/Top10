...
* 로깅 및 알림 이벤트를 사용자 또는 공격자에게 노출하거나([A01:2025-불충분한 접근 제어](A01_2025-Broken_Access_Control.md) 참조), 로깅되면 안 되는 민감정보(예: PII 또는 PHI)를 로깅함으로써 민감 정보 유출에 취약한 경우.
...
**시나리오 1:** 한 아동 건강보험 제공업체의 웹사이트 운영자는 모니터링 및 로깅 부재로 인해 침해 사고를 탐지하지 못했다. 외부 제3자가 해당 제공업체에 공격자가 350만 명이 넘는 아동의 건강 정보 기록 수천 건에 접근하여 이를 수정했다고 통보했다. 사후 분석에서는 웹사이트 개발자가 중대한 취약점을 장기간 방치된 정황이 확인되었으며, 시스템의 로그와 모니터링되고 있지 않아 침해가 2013년부터 7년 이상 지속되었을 가능성도 존재한다.
