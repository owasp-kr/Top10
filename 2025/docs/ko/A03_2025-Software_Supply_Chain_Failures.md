...
* 공급망 시스템에 직무 분리 사항이 없는 경우. 다른 사람의 감독 없이 공급망 내 코드를 작성하거나 운영 환경에 배포되는 전 과정을 당독으로 수행해선 안 된다.
...
* 시스템 개발, 배포보다 CI/CD 파이프라인 보안성이 약한 경우. (특히 파이프라인이 복잡할 때)
...
* 사용 중인 컴포넌트에 대한 취약점들을 보안 취약점(CVE), 국가적 취약점 데이터베이스(NVD), [오픈 소스 취약점(OSV)](https://osv.dev/) 을 통해 지속 모니터링한다. 이를 위해 소프트웨어 구성 분석, 소프트웨어 공급망, 프로세스 자동화를 위한 소프트웨어 구성 목록(SBOM) 도구 사용 또는 사용 중인 컴포넌트에 대한 보안 취약점 공지 구독을 통해서도 관리할 수 있다.
...
* 코드 저장소: 백업, 보호된 브런치, 민감 데이터 커밋 포함
* 개발자 워크스테이션: 주기적 패치, 다중 인증(MFA), 모니터링 등
* 빌드 서버와 CI/CD 서버: 직무 분리, 접근 제어, 서명된 빌드, 환경변수 내 민감 데이터, 로그 조작 방지 등
* 아티팩트: 무결성 보장을 위해 출처, 서명, 타임스탬프를 확인하고, 각 환경 재구축보단 아티팩트 재배포를 한다. 또한 빌드가 변경 불가능한지 검증한다.
...
**시나리오 1:** 신뢰된 공급업체가 멀웨어에 감염되어, 업그레이드되는 과정에서 사용자 시스템까지 침해되는 사례이다. 유명한 예시는 다음과 같다.



* 2019년 솔라윈즈 침해사고는 18,000개의 조직 침해사고로 이어졌다.[https://www.npr.org/2021/04/16/985439655/a-worst-nightmare-cyberattack-the-untold-story-of-the-solarwinds-hack](https://www.npr.org/2021/04/16/985439655/a-worst-nightmare-cyberattack-the-untold-story-of-the-solarwinds-hack)
...
**시나리오 3:** 2025년에 발생한 [`Shai-Hulud` 공급망 공격](https://www.cisa.gov/news-events/alerts/2025/09/23/widespread-supply-chain-compromise-impacting-npm-ecosystem)은 첫번째로 성공한 자기 전파형 npm 웜이다. 공격자는 유명한 패키지에 악성 프로그램 버전을 배포하였으며, 이는 post-install 스크립트를 이용해 민감 정보를 퍼블릭 깃허브 저장소로 유출하거나 탈취하는 역할을 한다. 해당 멀웨어는 피해자 환경 내에서 npm 토큰을 탐지하여 이를 이용해 자동적으로 접근 가능한 패키지의 악성 버전을 푸시한다. 해당 웜은 npm에 의해 차단되기 전 500개가 넘는 패키지 버전에 영향을 미쳤다. 이번 공급망 공격은 고도화되었으며, 빠르게 전파되고 심각한 피해를 입혔다. 그리고 개발자 환경을 직접 노림으로써 개발자 자신이 공급망 공격을 위한 주요 표적이 될 수 있다는 것을 보여주었다.

**시나리오 4:** 컴포넌트는 일반적으로 애플리케이션과 동일한 권한 실행되므로, 컴포넌트의 취약점이 심각한 영향을 미칠 수 있다. 해당 취약점은 사고적이거나(예: 코딩 에러), 고의적일 수 있다(예: 컴포넌트 내 백도어). 발견된 악용 가능한 컴포넌트 취약점 대표적 사례는 다음과 같다. 

* CVE-2017-5638, 스트럿츠(Struts) 버전 2에 원격 코드 실행 취약점이 발견되었으며, 이는 서버에 임의코드 실행이 가능한 심각한 보안 사고 원인으로 판별되었다.
